...
static struct dsi_cmd_desc dim_on_cmds[] = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 1, sizeof(dsi_dim_on), dsi_dim_on},
};

static struct dsi_cmd_desc dim_off_cmds[] = {
	{DTYPE_DCS_LWRITE, 1, 0, 0, 1, sizeof(dsi_dim_off), dsi_dim_off},
};
...
static char TMF[] = { 
		...
		0x23, 0x18, 0x2C, 0x16,
		0x00, 0x00};
static char ALPS[20] = {0xEA, 0x01, 0x00, 0x20, 0x40, 0x60, 0x80, 0xA0, 0xC0, 0xE0, 0xFF, 0x00, 0x20, 0x40, 0x60, 0x80, 0xA0, 0xC0, 0xE0, 0xFF};
...
static struct dsi_cmd_desc sharp_sre_ctrl_cmds[] = {
	  {DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(unlock), unlock},
	  {DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(SRE_Manual_0), SRE_Manual_0},
...
static void m7wl_dim_on(struct msm_fb_data_type *mfd)
{
	...
	if (panel_type == PANEL_ID_M7_SHARP_RENESAS_C1) {
		cmdreq.cmds = sharp_cmd_dim_on_cmds;
		cmdreq.cmds_cnt = ARRAY_SIZE(sharp_cmd_dim_on_cmds);
		...
		cmdreq.cmds = samsung_dim_on_cmds;
		cmdreq.cmds_cnt = ARRAY_SIZE(samsung_dim_on_cmds);
	} else if (panel_type == PANEL_ID_M7_JDI_RENESAS) {
		cmdreq.cmds = jdi_renesas_dim_on_cmds;
		cmdreq.cmds_cnt = ARRAY_SIZE(jdi_renesas_dim_on_cmds);
	} else {
...
static unsigned char m7wl_shrink_pwm(int val)
{
	...
	if ((panel_type == PANEL_ID_M7_JDI_SAMSUNG) ||
		...
		panel_type == PANEL_ID_M7_SHARP_RENESAS_C1 ||
...
static void m7wl_color_enhance(struct msm_fb_data_type *mfd, int on)
{
	if ((panel_type == PANEL_ID_M7_JDI_SAMSUNG) ||
		...
		cmdreq.cmds = samsung_color_enhance_on_cmds;
		cmdreq.cmds_cnt = ARRAY_SIZE(samsung_color_enhance_on_cmds);
		cmdreq.flags = CMD_REQ_COMMIT;
		if (mfd && mfd->panel_info.type == MIPI_CMD_PANEL)
			cmdreq.flags |= CMD_CLK_CTRL;
		...
		PR_DISP_INFO("switch SAE\n");
	} else if (panel_type == PANEL_ID_M7_SHARP_RENESAS) {
		if (on) {
			sharp_renesas_color_en[1] = 0x01;
		} else {
			sharp_renesas_color_en[1] = 0x00;
		}
		...
		PR_DISP_INFO("switch color enhance\n");
			...
			panel_type == PANEL_ID_M7_JDI_RENESAS) {
		if (on) {
			m7_Color_enhancement[1] = 0x01;
		} else {
			m7_Color_enhancement[1] = 0x00;
		}
...
static void m7wl_sre_ctrl(struct msm_fb_data_type *mfd, unsigned long level)
{
   ...
   if (prev_level != level) {
		...
		if(panel_type == PANEL_ID_M7_SHARP_RENESAS_C1 ||
			...
			cmdreq.cmds = sharp_sre_ctrl_cmds;
			cmdreq.cmds_cnt = ARRAY_SIZE(sharp_sre_ctrl_cmds);
			cmdreq.flags = CMD_REQ_COMMIT;
			if (mfd && mfd->panel_info.type == MIPI_CMD_PANEL)
			cmdreq.flags |= CMD_CLK_CTRL;
			cmdreq.rlen = 0;
			...
			cmdreq.cmds = jdi_samsung_sre_ctrl_cmds;
			cmdreq.cmds_cnt = ARRAY_SIZE(jdi_samsung_sre_ctrl_cmds);
			cmdreq.flags = CMD_REQ_COMMIT;
			if (mfd && mfd->panel_info.type == MIPI_CMD_PANEL)
			cmdreq.flags |= CMD_CLK_CTRL;
			cmdreq.rlen = 0;
...
static int __init mipi_video_sharp_init(void)
{
	...
	if (panel_type == PANEL_ID_DLXJ_SHARP_RENESAS) {
		strcat(ptype, "PANEL_ID_DLXJ_SHARP_RENESAS");
...
static int __init mipi_command_samsung_init(void)
{
	...
	if(panel_type == PANEL_ID_M7_JDI_SAMSUNG) {
		strcat(ptype, "PANEL_ID_M7_JDI_SAMSUNG");
		...
		strcat(ptype, "PANEL_ID_M7_JDI_SAMSUNG_C2");
